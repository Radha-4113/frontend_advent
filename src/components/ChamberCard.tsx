...
export function ChamberCard({ chamber, onStart, onStop, onResume, onSettings }: ChamberCardProps) {
  const statusColors = {
    active: '#28A745',
    fault: '#DC3545',
    paused: '#FFC107',
  ...
  const getDotConfig = () => {
    if (chamber.status === 'active') {
      return { color: '#28A745', position: chartData.length - 1 }; // Green at current position
    } else if (chamber.status === 'fault' || chamber.powerFailure) {
      return { color: '#DC3545', position: chartData.length - 1 }; // Red where it stopped
    } else if (chamber.status === 'paused' || chamber.powerRestored) {
      return { color: '#FFC107', position: chartData.length - 1 }; // Yellow for resume
    }
    return null;
  };
  ...
  return (
    <div className={`rounded-lg shadow-md overflow-hidden border border-gray-300 hover:shadow-lg transition-shadow ${
      ...
      <div className="p-5">
        ...
        {chamber.powerRestored && (
          <div className="mb-3 bg-[#FFC107] text-gray-900 px-3 py-2 rounded flex items-center gap-2">
            ...
            <span className="text-xs">Power Restored – Resume Cycle?</span>
          </div>
        )}
              ...
              <LineChart data={chartData}>
                <Line
                  ...
                  dot={(props) => {
                    ...
                    // Only show the dot at the position determined by status
        ...
        {chamber.powerRestored && (
          <div className="mb-3 bg-yellow-50 border border-yellow-200 text-gray-800 px-3 py-1.5 rounded text-xs">
            Resume for 45 minutes at 120°C (Predicted Compensation Applied)
          </div>
        )}
        ...
        <div className="flex gap-2">
          {chamber.powerRestored ? (
            <button
              onClick={() => onResume(chamber.id)}
              className="flex-1 bg-[#FFC107] hover:bg-[#E0A800] text-gray-900 py-1.5 px-3 rounded text-sm transition-colors"
            >
              Resume Cycle
            </button>
          ) : (
